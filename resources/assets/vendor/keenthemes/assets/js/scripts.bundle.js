		...
		var Plugin = {
			...
			paging: function(meta, callback) {
				var pg = {
                    ...
                    cal_pagination: function (c, m, d) {
                        var pagesNumber = Plugin.getOption('toolbar.items.pagination.pages.desktop.pagesNumber');
                        var current = c,
                            last = m,
                            delta = Math.floor(d / 2),
                            left = current - delta,
                            right = current + delta,
                            range = [],
                            rangeWithDots = [],
                        ...
                        if(current > last - delta) left = last - delta * 2;
                        if(current < delta + 1) right = 1 + delta * 2;
					...
					populate: function() {
						...
						var pagesNumber = Plugin.getOption('toolbar.items.pagination.pages.desktop.pagesNumber');
                        var items = this.cal_pagination(pg.meta.page, pg.meta.pages, pagesNumber);

                        items.forEach(function(pageNumber) {
                            if(pageNumber === '...') {
                                $('<li/>').
                                    append($('<a/>').
                                    addClass(pfx + 'datatable-pager-link-disabled ' + pfx + 'datatable-pager-link').
                                    attr('disabled', true).
                                    text(pageNumber).
